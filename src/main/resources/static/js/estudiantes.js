...
async function cargarEstudiantes() {
    ...
    try {
        let url = `${API_BASE_URL}/api/estudiantes`;

        // Si es MONITOR, usar su zona y jornada
        if (currentUser.rol === 'MONITOR' && monitorData && monitorData.zonaId && monitorData.jornadaId) {
            url = `${API_BASE_URL}/api/estudiantes/monitor/zona/${monitorData.zonaId}/jornada/${monitorData.jornadaId}`;
        }
...
async function guardarEstudiante() {
    ...
    try {
        const url = modoEdicion ? `${API_BASE_URL}/api/estudiantes/${estudianteId}` : `${API_URL}/estudiantes`;
