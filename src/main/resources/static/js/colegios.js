...
async function cargarZonas() {
    try {
        const response = await fetch(`${API_BASE_URL}/api/zonas/activas`, {
...
async function cargarColegios() {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/api/colegios`, {
            headers: Auth.getHeaders()
        });
...
async function cargarJornadasParaCrear() {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/api/jornadas/zona/${zonaId}/activas`, {
            headers: Auth.getHeaders()
        });
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (jornadasSeleccionadas.length > 0) {
                try {
                    await fetch(`${API_BASE_URL}/api/colegio-jornadas/colegio/${colegioCreado.id}/asignar-varias`, {
                        method: 'POST',
                        headers: Auth.getHeaders(),
...
async function editarColegio(id) {
    try {
        const response = await fetch(`${API_BASE_URL}/api/colegios/${id}`, {
            headers: Auth.getHeaders()
        });
...
async function actualizarColegio() {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/api/colegios/${colegioId}`, {
            method: 'PUT',
            headers: Auth.getHeaders(),
            body: JSON.stringify(dto)
        });
...
async function verDetalle(id) {
    try {
        const response = await fetch(`${API_BASE_URL}/api/colegios/${id}/estadisticas`, {
            headers: Auth.getHeaders()
        });
...
async function cargarJornadasAsignadas(colegioId) {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/api/colegio-jornadas/colegio/${colegioId}`, {
            headers: Auth.getHeaders()
        });
...
async function agregarJornada() {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/api/colegio-jornadas`, {
            method: 'POST',
            headers: Auth.getHeaders(),
            body: JSON.stringify(dto)
        });
...
async function toggleJornada(colegioJornadaId, activar) {
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/api/colegio-jornadas/${colegioJornadaId}/${endpoint}`, {
            method: 'PATCH',
            headers: Auth.getHeaders()
        });
...
async function quitarJornada(colegioJornadaId) {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/api/colegio-jornadas/${colegioJornadaId}`, {
            method: 'DELETE',
            headers: Auth.getHeaders()
        });
...
async function cambiarEstado(id, activar) {
    ...
    try {
        const endpoint = activar ? 'activar' : 'desactivar';
        const response = await fetch(`${API_BASE_URL}/api/colegios/${id}/${endpoint}`, {
            method: 'PATCH',
            headers: Auth.getHeaders()
        });
